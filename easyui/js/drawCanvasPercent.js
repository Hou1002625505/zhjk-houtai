(function($) {
				$.fn.drawCanvasPercent = function(options) {
					//各种属性、参数
					var defaults = {
						type: 2, //类型默认1，有[1,2,3]
						dw: 'px', //判断是单位是rem还是px
						cir_r: 1, //圆饼的直径
						cir_color: '#f2d062', //圆饼的占比颜色
						line_w: 0.16, //圆饼的线条宽度
					...
					this.each(function() {
						...
						var cur_obj = $(this);
						if(options.dw == "px") {
							var cur_cir_r = options.cir_r * (window.screen.width / 23);
							var cur_line_w = options.line_w * (window.screen.width / 23);
						} else {
